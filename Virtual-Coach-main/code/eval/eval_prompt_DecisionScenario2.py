...
try:
    ...
    from data.prompt.DecisionScenario2 import DecisionScenario2
...
eval_dataset_file = os.path.join(ROOT_DIR, 'data', 'eval', 'DecisionScenario2.json')
...
eval_dataset_result_file = os.path.join(ROOT_DIR, 'data', 'eval_result', 'DecisionScenario2_result_' + timestamp + '.json')
...
def evaluate_accuracy():
    ...
    def process_sample(i):
        ...
        temp_data_prompt = DecisionScenario2.format(
        ...
        if response_json is not None and successful_times is not None:
            ...
            try:
                if response_json["need_to_communicate"] == ground_truth["need_to_communicate"]:
                    temp_data_result["is_correct"] = True
                else:
                    reason_str = f"真实={ground_truth['need_to_communicate']}, 预测={response_json['need_to_communicate']}"
