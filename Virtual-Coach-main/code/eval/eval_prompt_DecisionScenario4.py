...
try:
    ...
    from data.prompt.DecisionScenario4 import DecisionScenario4
...
eval_dataset_file = os.path.join(ROOT_DIR, 'data', 'eval', 'DecisionScenario4.json')
...
eval_dataset_result_file = os.path.join(ROOT_DIR, 'data', 'eval_result', 'DecisionScenario4_result_' + timestamp + '.json')
...
def evaluate_accuracy():
    ...
    def process_sample(i):
        ...
        temp_data_prompt = DecisionScenario4.format(
            training_performance=temp_data["training_performance"],
            set_reps_next=temp_data["set_reps_next"],
            prev_service=temp_data["prev_service"],
        ...
        if response_json is not None and successful_times is not None:
            ...
            try:
                if response_json["service_name"] == ground_truth["service_name"]:
                    temp_data_result["is_correct"] = True
                else:
                    reason_str = f"真实={ground_truth['service_name']}, 预测={response_json['service_name']}"
