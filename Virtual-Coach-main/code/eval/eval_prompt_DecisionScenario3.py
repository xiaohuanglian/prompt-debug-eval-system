...
try:
    ...
    from data.prompt.DecisionScenario3 import DecisionScenario3
...
eval_dataset_file = os.path.join(ROOT_DIR, 'data', 'eval', 'DecisionScenario3.json')
...
eval_dataset_result_file = os.path.join(ROOT_DIR, 'data', 'eval_result', 'DecisionScenario3_result_' + timestamp + '.json')
...
def evaluate_accuracy():
    ...
    def process_sample(i):
        ...
        temp_data_prompt = DecisionScenario3.format(
            training_performance=temp_data["training_performance"],
            communication_result=temp_data["communication_result"],
            user_goal=temp_data["user_goal"],
            exercise_name=temp_data["exercise_name"],
            set_reps=temp_data["set_reps"],
            quality_checkpoints=temp_data["quality_checkpoints"]
        )
        ...
        if response_json is not None and successful_times is not None:
            ...
            try:
                if response_json["should_adjust"] == ground_truth["should_adjust"]:
                    temp_data_result["is_correct"] = True
                else:
                    reason_str = f"真实={ground_truth['should_adjust']}, 预测={response_json['should_adjust']}"
