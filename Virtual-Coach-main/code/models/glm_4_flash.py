...
# -*- coding: utf-8 -*-
"""
LLM Gateway API 模型调用文件 - GLM-4-Flash
    ...
    while tried < max_retries:
        try:
            ...
            if status_code == 503:
                ...
                delay = base_delay * (2 ** tried)
                last_error = f"HTTP 429 (请求频率过高)"
                print(f"[Retry {tried + 1}/{max_retries}] 频率限制，等待 {delay}s 后重试...")
                tried += 1
                time.sleep(delay)
                continue
            elif status_code != 200:
                last_error = f"HTTP {status_code}"
                print(f"[Retry {tried + 1}/{max_retries}] {last_error}: {response.text[:200]}")
                tried += 1
